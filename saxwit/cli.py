...
# Must run before any aiohttp-based module (socialscan, maigret) opens a
# connection — see dns_fix.py for why this is needed on Termux/Android.
from saxwit import dns_fix  # noqa: F401
...
_RATE_LIMIT_BOILERPLATE = (
    " You might be sending too many requests. Use a proxy or wait before trying again."
)


def _shorten(text: str, limit: int = 46) -> str:
    """Trim noisy boilerplate and cap length so table rows stay one line tall."""
    if not text:
        return "-"
    text = text.replace(_RATE_LIMIT_BOILERPLATE, "").strip()
    if len(text) > limit:
...
def run_email_osint(email: str) -> None:
    ...
    table = Table(box=box.SIMPLE_HEAVY, title="Platforms that recognize this email", expand=True)
    table.add_column("Platform", style="accent", no_wrap=True, ratio=1)
    table.add_column("Extra info", style="white", no_wrap=True, overflow="ellipsis", ratio=2)
    ...
    if used:
        for r in used:
            ...
            table.add_row(r.get("domain", r.get("name", "?")), _shorten(" | ".join(extra_bits), limit=60))
        console.print(table)
    else:
...
def run_phone_osint(country_code: str, national_number: str) -> None:
    ...
    table.add_column("Service", style="accent", no_wrap=True, ratio=1)
...
def run_username_maigret(username: str, top_sites: int, timeout: int) -> None:
    ...
    table = Table(box=box.SIMPLE_HEAVY, title=f"Accounts found for '{username}'", expand=True)
    table.add_column("Site", style="accent", no_wrap=True, ratio=1)
    table.add_column("URL", style="info", no_wrap=True, overflow="ellipsis", ratio=2)
    table.add_column("Tags", style="dim", no_wrap=True, overflow="ellipsis", ratio=1)
...
def run_socialscan(queries_raw: str) -> None:
    ...
    table = Table(box=box.SIMPLE_HEAVY, expand=True)
    table.add_column("Query", style="accent", no_wrap=True, overflow="ellipsis", ratio=2)
    table.add_column("Platform", style="white", no_wrap=True, width=10)
    table.add_column("Status", style="white", no_wrap=True, width=9)
    table.add_column("Details", style="dim", no_wrap=True, overflow="ellipsis", ratio=3)
    ...
    for r in results:
        ...
        table.add_row(r["query"], r["platform"], status, _shorten(r["message"]))
